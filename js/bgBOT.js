class BbtanBgBot {
  ...
  drawBbtanBgBot(gameStatus, game) {
    ...
    this.ctx.drawImage(this.bGBotImage,0,0,this.bGBotImage.width,this.bGBotImage.height,this.x,this.y,GAME_WIDTH,BOT_BG_HEIGHT);
  ...
  drawCoin(gameStatus,game){
    if(gameStatus == 'inGame' || gameStatus == 'nextLevel') {
      ...
      this.ctx.drawImage(game.spriteSheet,sx,sy,POWER_UPS_SIZE,POWER_UPS_SIZE,coinX,coinY,POWER_UPS_WIDTH,POWER_UPS_HEIGHT);
      this.ctx.fillText(game.coin,coinX*2 + POWER_UPS_SIZE,this.y + BOT_BG_HEIGHT/1.6);
    }
  }
